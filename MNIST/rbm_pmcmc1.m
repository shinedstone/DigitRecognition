...
for epoch = 1:maxepoch
    ...
    errsum = 0;
    epsilonw      = 0.01/(1+epoch/2000);
    epsilonvb     = 0.01/(1+epoch/2000);
    epsilonhb     = 0.01/(1+epoch/2000);
    ...
    if epoch > maxepoch*0.1
        [ falsedata, falsetarget, nBatches ] = make_samples(vishid,hidbiases,visbiases,labhid,labbiases,batchdata,batchtargets);
        ...
        for batch = 1 : nBatches
            ...
            %%%%%%%%% UPDATE WEIGHTS AND BIASES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            vishidinc = 0.1*epsilonw*(posprods/numcases-negprods/nchain);
            visbiasinc = 0.1*epsilonvb*(posvisact/numcases-negvisact/nchain);
            hidbiasinc = 0.1*epsilonhb*(poshidact/numcases-neghidact/nchain);
            labhidinc = 0.1*epsilonw*(poslabprods/numcases-neglabprods/nchain);
            labbiasinc = 0.1*epsilonvb*(poslabact/numcases-neglabact/nchain);
    ...
    if rem(log2(epoch),1) == 0
        RBMClassification = [ RBMClassification calculate_classification(vishid,hidbiases,visbiases,labhid,labbiases,testbatchdata,testbatchtargets) ];
        figure(4);
