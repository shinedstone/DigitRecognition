...
for epoch = 1:maxepoch
    ...
    for batch = 1:numbatches
        ...
        neghidstates = 1./(1 + exp(-(negdata.*repmat(temperature', 1, numdims))*vishid-temperature'*hidbiases)) > rand(nchain, numhid);
        
        negdata = 1./(1 + exp(-(neghidstates.*repmat(temperature', 1, numhid))*vishid'-temperature'*visbiases)) > rand(nchain, numdims);
